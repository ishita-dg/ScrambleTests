...
% TODO: Create classes of tests based on length of subject/distance across swaps, and ensure that pairs are of two kinds. Check statistical validity of success in InferSent.
% TODO: Devise a permutation test – does InferSent just detect the reshuffling and assign perturbed label? Or are the perturbations dependable? Not sure if reasonable to draw inferences from InferSent on gibberish permutations... Maybe just check if vector between original and shuffled can be distinguished in cases where the shuffle takes you to different other classes. 
...
% This indicates that InferSent largely relies on permutations of the same words often indicating contradictions (at least in its training set), rather than true compositionality.
...
\hline
 ...
 \hline
  \end{tabular}}
 \caption{High Overlap Data Statistics}
  \label{tab:highMargin}
\end{table}As shown in the table, when there are more overlap between two sentences, it's more likely to be entailment or neutral and less likely to be contradiction. Notably, there are only $8.5\%$ contradictory sentence pairs in top 1000 overlap data. }
...
\hline 
 ...
 \caption{Overall Data Negation statistics}
  \label{tab:highMargin}
\end{table}
Here, we denote $N$ as negation, $E$ as entailment and $C$ as contradiction. As we can see, negation correlates more with contradictions than entailment almost 3 times.\\
Moreover, in top 10,000 overlap data (relevant), in SNLI, we observe that $60.0 \%$ sentence pairs with negation are contradictions, and only $7.5 \%$ are entailment. \\
...
From qualitative analysis, we observe that if common antonyms are swapped in order, then Infersent will likely classify this sentence pair as a contradiction. For example:\\
 A:"Boy sitting with a girl on the bench in the park"\\
...
(TODO:verify using quantitative data that my above claims are correct)}
